...
namespace osu.Android
{
    ...
    public class ModMenuOverlay : Java.Lang.Object, IDisposable
    {
        ...
        public void Show()
        {
            ...
            if (menuLayout != null)
            {
                // Overlay уже создан (например после Hide) — просто делаем триггер видимым
            ...
            try
            {
                ...
                // OnStateChanged вызывается из ThreadPool — updateButtons
                // оборачивается в RunOnUiThread (см. updateButtonsOnUiThread).
        ...
        /// Views остаются в WindowManager — не пересоздаём их при возврате.
        /// </summary>
        public void Hide()
        {
            ...
            try
            {
                ...
                global::Android.Util.Log.Debug("ModMenuOverlay", "Overlay hidden");
            }
            catch (Exception ex)
            {
        ...
        private void createMenu()
        {
            ...
            try
            {
                ...
                // Первоначальное обновление кнопок — мы на UI потоке, вызываем напрямую
                updateButtons();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Обёртка для updateButtons — маршалирует вызов на UI поток.
        /// OnStateChanged стреляет из ThreadPool, трогать View
        /// напрямую оттуда нельзя — CalledFromWrongThreadException.
        /// </summary>
        private void updateButtonsOnUiThread()
        {
            ...
            {
                global::Android.Util.Log.Warn("ModMenuOverlay", "Context is not an Activity, updating buttons directly");
                updateButtons();
            }
        }
        ...
        /// Должен вызываться ТОЛЬКО из UI потока.
        /// </summary>
        private void updateButtons()
        {
            try
            {
                ...
                if (noMissButton != null)
                {
                    ...
                    // Блокируем кнопку только когда AutoPlay активен
        ...
        /// БАГ FIX: убрана дублирующаяся ветка >= O — она возвращала то же самое,
        /// что >= M, и никогда не имела смысла как отдельная проверка.
        /// </summary>
...
#pragma warning restore CS0618
        ...
        private void cleanupViews()
        {
            try
            {
                ...
                // БАГ FIX: windowManager получен через JavaCast — Java-объект,
                // его нужно явно освобождать, иначе утечка нативного объекта.
